\begin{sidewaystable}[htbp]
	...
	\begin{tabular}{@{}m{5cm}rm{10cm}@{}}
		...
		&  &  \\
		Attn\textsubscript{vertical} (dB) & $0.5 \times (1 + 1) = 1.00$ & Same as notes for Attn\textsubscript{horizontal}, but for x\textsubscript{1} = 300~mm, x\textsubscript{2}~=~400~mm and a duct length of 0.5~m \\
		&  &  \\
		...
		&  &  \\
		L\textsubscript{W\textsubscript{out}} (dB re 10\textsuperscript{-12} W) & $92 - 62.37 = 29.63$ & Sound power level leaving single ductwork: $L_{W out} = L_{W fan} - \Sigma Attn$ \\
		&  &  \\
		T\textsubscript{1} (s) & 0.48 & From Table~\ref{tbl:reverb_conf} \\
		&  &  \\
		SPL in conference room due to single ductwork, L\textsubscript{p\textsubscript{1 single}} (dB) & $29.63 - 10 log \frac{80}{0.48} + 14 = 21.4$ & \begin{tabular}[c]{@{}l@{}} $L_{p_1 single} = L_{W out} – 10 log \frac{V_1}{T_{1}} + 14$\end{tabular} \\
		&  &  \\
		SPL in conference room due to supply and return, L\textsubscript{p\textsubscript{1}} (dB) & $21.4 + 3 = 24.4$ & Sum of two identical SPLs = $L_p + 3$ \\ \bottomrule
	\end{tabular}
\end{sidewaystable}